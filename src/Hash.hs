...
hashIO ptr bytes salt = with (fromIntegral salt) $ \sp -> do
  ...
  peek sp
 where
  go p1 p2 | bytes .&. 3 == 0 = hashWord2 (castPtr ptr) words p1 p2
...
hashList salt xs = withArrayLen xs
  $ \len ptr -> hashIO ptr (fromIntegral (len * sizeOf x)) salt
  where x = head xs
...
rechunk s
  ...
  = []
  | otherwise
  = let (pre, suf) = Lazy.splitAt chunkSize s in repack pre : rechunk suf
 where
